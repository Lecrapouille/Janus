...
std::vector<bool> Menu::centered_buttons(const std::vector<std::string> texts,
                                         float buttons_height,
                                         float spaces_size)
{
    float window_width = ImGui::GetWindowSize().x;
    float buttons_widths =
        (window_width - ((texts.size() + 2) * spaces_size)) / texts.size();
    ...
    {
        ImGui::SameLine();
        ImGui::SetCursorPosX(spaces_size + buttons_widths * i +
                             spaces_size * i);
        ImGui::Text("%s", "");
        ImGui::SameLine();
        result[i] = ImGui::Button(texts[i].data(),
                                  ImVec2(buttons_widths, buttons_height));
    }
...
void Menu::galaxy()
{
    ...
    ImGui::SliderInt("##nb_stars",
                     &nb_stars,
                     1000,
                     1000000,
                     NULL,
                     ImGuiSliderFlags_Logarithmic);
    ImGui::ColorPicker4("##color_positive_mass",
                        (float*)&Menu::color_positive_mass,
    ...
    ImGui::SliderFloat(
        "##galaxy_diameter", &galaxy_diameter, 10.f, 1000.f, "%.0f");

    ImGui::NewLine();

    ImGui::Text("Thickness of the galaxy");
    ImGui::SliderFloat(
        "##galaxy_thickness", &galaxy_thickness, 1.f, 100.f, "%.0f");
...
void Menu::collision()
{
    ...
    ImGui::SliderFloat(
        "##galaxies_distance", &galaxies_distance, 10.f, 1000.f, "%.0f");
...
void Menu::universe()
{
    ...
    ImGui::SliderFloat(
        "##galaxy_diameter", &galaxy_diameter, 1.f, 100.f, "%.1f");
...
void Menu::display()
{
    ...
    if (visible)
    {
        ...
        title("Simulation");
        std::string timing =
            "Iterations: " + std::to_string(Computer::simulation_iterations) +
            ", time: " + std::to_string(Computer::simulation_time);
        ImGui::Text("%s", timing.c_str());

        std::vector<bool> buttons =
            centered_buttons({ "Reset", "Restart", pause_button }, 25.f, 5.f);
        if (buttons[2])
        {
            pause = !pause;
        }
        ...
        ImGui::Text("What");
        ImGui::Combo("##simulation_type",
                     reinterpret_cast<int*>(&simulation_type),
                     "Galaxy\0Collision\0Universe\0");
        ...
        ImGui::Combo("##simulation_model",
                     reinterpret_cast<int*>(&simulation_model),
                     "Black Hole\0Newton\0Anti Newton\0");
        ...
        if (simulation_model == SimulationModel::Old)
        {
            ...
            ImGui::SliderFloat("##black_hole_mass",
                               &black_hole_mass,
                               0.f,
                               1000000.f,
                               "%.0f",
        ...
        else
        {
            ...
            ImGui::SliderFloat("##negative_mass_proportion",
                               &negative_mass_proportion,
                               0.0f,
                               1.0f,
                               "%.4f");

            ImGui::ColorPicker4("##color_negative_mass",
                                (float*)&Menu::color_negative_mass,
                                ImGuiColorEditFlags_NoSidePreview |
                                    ImGuiColorEditFlags_NoSmallPreview);
        }
        ...
        ImGui::SliderFloat("##smoothing_length",
                           &smoothing_length,
                           0.001f,
                           1.f,
                           NULL,
        ...
        ImGui::SliderFloat("##interaction_rate",
                           &interaction_rate,
                           0.001f,
                           1.f,
                           "%.3f",
